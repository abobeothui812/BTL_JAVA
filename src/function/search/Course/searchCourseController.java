...
public class searchCourseController extends TeacherController{
    ...
    private List<Class> getClassDetails(String courseID) {
        ...
        try {
                ...
                while (resultSet.next()) {
                    hasResults = true;
                    classList.add(new Class(resultSet.getInt("ClassID"), new Course(resultSet.getString("CourseID"), CourseNameInit, CourseCreditInit), resultSet.getInt("RegisteredStudents"), resultSet.getString("Schedule")));
                }
    ...
    private VBox createClassBox(Class classDetails) {
        ...
        Text headline = new Text(classDetails.getClassID() + " - " + classDetails.getCourse().getCourseName() + " - " + classDetails.getCourse().getCourseID());
